...
TryToGetMedicalTreatment = function(medicIndex)
    ...
    Heap.ESX.TriggerServerCallback("chames_ambulance_medic:requiredAmount", function(required)
        if required then
            ...
            Heap.ESX.ShowNotification("There are currently medics in duty, talk to them first please.")
        end
    end)
end
...
BedThread = function(medicIndex, bed, bedHandle)
    ...
    while GetGameTimer() - Heap.InBed < Default.HelpTime do
        ...
        DrawScriptText(bed.Location, "Getting ~b~treatment~s~ " .. timePercent .. "%")
    end
    ...
    Heap.ESX.ShowNotification("You've been treated, come back any time if you need help.")
end
